...
const FormPage = () => {
  const [formData, setFormData] = useState({
    ...
    category: "",
    codeOffice: "",
    description: "", // Masih ada di sini tetapi tidak required
    proof: null, // State untuk file bukti
  });
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...formData,
      [name]: type === "file" ? files[0] : value, // Menangani file
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        for (const key in formData) {
            // Hanya tambahkan proof jika ada file yang dipilih
